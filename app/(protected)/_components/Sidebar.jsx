...
const Sidebar = ({ isOpen, onToggle }) => {
  ...
  const menuItems = [
    ...
    { icon: <SchoolIcon />, text: 'Bài tập', path: '/dashboard/assignments?role=INSTRUCTOR' },
    { icon: <PeopleIcon />, text: 'Học viên', path: '/instructor-dashboard/' },
    { icon: <PaymentIcon />, text: 'Thanh toán', path: '/instructor-dashboard/' },
    { icon: <SettingsIcon />, text: 'Cài đặt', path: '/instructor-dashboard/' },
  ];
      ...
      `}>
        <div className="p-4 flex flex-col h-full">
          <div className="flex items-center justify-between mb-8">
            <h2 className={`text-xl font-bold transition-all duration-200 ${!isOpen ? 'opacity-0 w-0' : 'opacity-100 w-auto'}`}>
              <span className="text-blue-600 cursor-pointer" onClick={() => router.push('/courses')}>Rooy</span> Panel
            </h2>
